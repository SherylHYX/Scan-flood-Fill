...
INPUT_PATH = 'F:/fill_boundary/MSRA10K_Imgs_GT/MSRA10K_Imgs_GT/Imgs_gt'
...
def fixBoundary(img, startPoint, direction, length, boundaryColour):
...
def mask2edge(inputPath, outputPath):
    ...
    fixBoundary(result, [0, 0], [0, 1], column, 255)
    fixBoundary(result, [row - 1, 0], [0, 1], column, 255)
    fixBoundary(result, [0, 0], [1, 0], row, 255)
    fixBoundary(result, [0, column - 1], [1, 0], row, 255)
